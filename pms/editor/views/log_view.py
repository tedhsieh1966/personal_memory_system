...
class LogView(ViewBase):
    ...
    def _build(self) -> None:
        ...
        toolbar.grid(row=0, column=0, sticky="ew", padx=16, pady=(14, 6))

        ctk.CTkLabel(toolbar, text="Log",
                     font=ctk.CTkFont(size=18, weight="bold")).pack(side="left")

        ctk.CTkButton(toolbar, text="Refresh", width=80,
                      command=self._load_log).pack(side="left", padx=(16, 4))

        ctk.CTkButton(toolbar, text="Browse…", width=80,
                      command=self._browse_log).pack(side="left", padx=4)

        self._auto_var = tk.BooleanVar(value=False)
        self._auto_chk = ctk.CTkSwitch(
            toolbar, text="Auto-refresh", variable=self._auto_var,
            command=self._toggle_auto,
        )
        self._auto_chk.pack(side="left", padx=12)

        ctk.CTkLabel(toolbar, text="Filter:").pack(side="left", padx=(12, 4))
        self._level_var = tk.StringVar(value="ALL")
        ctk.CTkComboBox(
            toolbar, variable=self._level_var,
            values=["ALL", "INFO", "WARNING", "ERROR", "DEBUG"],
            width=100, command=lambda _: self._apply_filter(),
        ).pack(side="left")

        # ── Log path field ─────────────────────────────────────────────────
        path_row = ctk.CTkFrame(self, fg_color="transparent")
        path_row.grid(row=1, column=0, sticky="ew", padx=16, pady=(0, 4))
        ctk.CTkLabel(path_row, text="File:").pack(side="left")
        self._path_var = tk.StringVar(value=str(Path.cwd() / _DEFAULT_LOG))
        ctk.CTkEntry(path_row, textvariable=self._path_var, width=480).pack(
            side="left", padx=8
        )
        ctk.CTkButton(path_row, text="Load", width=60,
                      command=self._load_log).pack(side="left")

        # ── Text box ───────────────────────────────────────────────────────
        self._textbox = ctk.CTkTextbox(self, state="disabled",
                                       font=ctk.CTkFont(family="Consolas", size=11))
        self._textbox.grid(row=2, column=0, sticky="nsew", padx=16, pady=(0, 12))
    ...
    def _load_log(self) -> None:
        ...
        threading.Thread(target=lambda: self.after(0, lambda: None), daemon=True).start()
        try:
            lines = []
            with open(path, "r", encoding="utf-8", errors="replace") as f:
                lines = f.readlines()
    ...
    def _apply_filter(self) -> None:
        ...
        if level == "ALL":
            ...
            filtered = [ln for ln in self._all_lines if level in ln]
