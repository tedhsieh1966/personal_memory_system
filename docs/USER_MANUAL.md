...
**NSSM** (optional, for Windows service): Download from [nssm.cc](https://nssm.cc/download), unzip, and add to your `PATH`. Without NSSM, the installer still copies the application files but cannot install the auto-start service — you can start `pms_api.exe` manually instead.
...
1. Copy `pms_api.exe`, `pms_editor.exe`, `pms_manager.exe`, and `config.yaml` to `%APPDATA%\pms`
2. Install and start the `pms_api` Windows service (set to auto-start at every boot)
...
| `pms_manager log [n]` | Print the last `n` lines of `pms_api.log` (default: 50) |
...
**NSSM**（可選，用於 Windows 服務）：從 [nssm.cc](https://nssm.cc/download) 下載，解壓縮並加入 `PATH`。若沒有 NSSM，安裝程式仍會複製應用程式檔案，但無法安裝自動啟動服務——您可改為手動啟動 `pms_api.exe`。
...
1. 將 `pms_api.exe`、`pms_editor.exe`、`pms_manager.exe` 和 `config.yaml` 複製到 `%APPDATA%\pms`
2. 安裝並啟動 `pms_api` Windows 服務（設定為每次開機自動啟動）
3. 若已勾選，拉取 Ollama 模型
4. 若已勾選，建立桌面捷徑
...
`start`、`stop` 和 `restart` 控制 **PMS API Windows 服務**（`pms_api.exe`）——負責記憶體儲存、搜尋、瀏覽器擷取和檔案監控的背景程序。這些指令不會影響 Ollama 或 NSSM。NSSM 不是持續運行的程序；它僅在安裝時作為服務包裝器，之後不再介入。Ollama 作為獨立程序運行，有專屬指令如下。
...
| `pms_manager log [n]` | 列印 `pms_api.log` 的最後 `n` 行（預設：50） |
...
日誌視圖追蹤 `pms_api.log`（或您指定的任何日誌檔案）。使用**篩選**下拉選單僅顯示 INFO、WARNING、ERROR 或 DEBUG 行。切換**自動重新整理**以每 2 秒自動更新視圖。
...
- `dist/pms_api.exe` — 作為單一執行檔的 API 伺服器
...
服務將標準輸出寫入執行檔所在資料夾中的 `pms_api.log`，將標準錯誤寫入 `pms_api_err.log`。
...
API 伺服器未執行，或在不同的連接埠上執行。啟動 `run_api.py` 並檢查編輯器頂部列中的 URL 是否與 `config.yaml` 中的 `api.host` 和 `api.port` 匹配。
...
可能沒有足夠的 STM 事件形成有意義的批次（最少約 5 個事件），或 AI 以意外格式返回回應。查看 `pms_api.log` 中的整合輸出。確保 Ollama 中已拉取 `qwen2.5:7b`（或您設定的模型）。

### 服務已安裝但 API 未啟動

查看部署資料夾中的 `pms_api_err.log`。常見原因：`config.yaml` 有語法錯誤、連接埠已被使用，或執行檔路徑含有空格而 NSSM 未給出帶引號的路徑。以系統管理員身份重新執行 `install_service.bat`。
